...
aop就是面向切面编程，核心思想是把哪些散布在各处的公共行为（比如日志，权限）抽出来，进行模块化复用

### 2. 为什么要用AOP

因为如果不用AOP,我们就需要在每个方法中都加入公共方法，直接侵入了原本的方法，违法了开闭原则。而且后续要修改时每个地方都要修改

### 3. AOP怎么使用

#### 1. 概念定义：

	1. Aspect: 就是我封装的那个‘通用功能’类，用于声明一个切面
...
Spring AOP 就是基于动态代理的，如果要代理的对象，实现了某个接口，那么 Spring AOP 会使用 **JDK Proxy**，去创建代理对象，而对于没有实现接口的对象，就无法使用 JDK Proxy 去进行代理了，这时候 Spring AOP 会使用 **Cglib** 生成一个被代理对象的子类来作为代理。不过在spring 2.0以后，及时实现了接口，默认也会用Cglib去创建代理对象，因为Cglib需要依赖接口，且性能有了较大的提升
